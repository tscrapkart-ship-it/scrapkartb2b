  ...
  return (
    <header className="sticky top-0 z-50 border-b-2 border-[var(--ink)] bg-[var(--paper)]">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-2.5 font-display text-base">
          <Image src="/logos/ScrapKart Black Logo.png" alt="ScrapKart" width={170} height={48} priority />
          <span className="border-2 border-[var(--ink)] px-2 py-0.5 font-mono text-[10px] tracking-[0.14em]">B2B</span>
        </Link>
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="font-sans text-xs font-bold uppercase tracking-[0.08em] text-[var(--ink)] px-3 py-2 border-2 border-transparent hover:border-[var(--ink)] hover:bg-[var(--green-tint)] transition-colors"
            >
              {link.label}
            </Link>
          ))}
